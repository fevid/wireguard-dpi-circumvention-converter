...
function generateAmneziaDefaults() {
  ...
  const isVersion15 = getById('junkAmnezia15')?.checked;
  ...
  const configs = {
    ...
    junkAmnezia15: { jc: 3, jmin: 1, jmax: 3 } 
  };
...
function convertToClashProxy(wgConfig, fileName) {
  ...
  for (const key of AMNEZIA_KEYS) {
    if(interfaceData.amneziaOptions[key] || peerData.amneziaOptions[key] || defaultAmnezia[key]  )
      if (key === 'i1') {
        // Special case: preserve I1 from file for AmneziaWG 1.5
        amneziaOptions[key] = interfaceData.amneziaOptions[key] 
          || peerData.amneziaOptions[key] 
          || defaultAmnezia[key];
      } else {
        // For jc, jmin, jmax, h1-h4: use user selection FIRST
        amneziaOptions[key] = defaultAmnezia[key]  // User selection first
          || interfaceData.amneziaOptions[key] 
          || peerData.amneziaOptions[key];
      }
  }
